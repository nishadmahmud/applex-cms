...
export default function PaymentMethods({
  ...
  // 🧩 Local temporary states so parent amounts don't change until Save
  const [tempPayAmount, setTempPayAmount] = useState(payAmount);
  const [tempMethods, setTempMethods] = useState(paymentMethods);
  ...
  const addPaymentMethod = () => {
    ...
    console.log("tempPayAmount...before in add..  ", tempPayAmount)
    setPaymentMethods([...paymentMethods, newMethod]);
    console.log("tempPayAmount...after in add..  ", tempPayAmount)
  };
  ...
  // 🟩 Initialize main amount with total payable when modal opens (commented to fix arif's issue)
  // useEffect(() => {
  ...
  // 🧷 Prevent premature "paid" state: only clear when total hits 0,
  // do NOT auto‑fill payAmount on modal open.
  useEffect(() => {
    if (Number(total) === 0 && Number(payAmount) !== 0) {
      setPayAmount(0);
    }
  }, [total]);
  ...
  },[paymentMethods, tempMethods, tempPayAmount])

  useEffect(() => {
    // 🧩 Sync modal each time it opens with latest parent data.
    // Prefill the Amount field to show the current total payable as suggestion,
    // but don't push it to the parent until "Save" is pressed.
    if (
      (Number(payAmount) == 0 || Number(payAmount) == 0.0) &&
      Number(total) > 0
    ) {
      if(tempPayAmount == 0){
        setTempPayAmount(Number(total)); // show Total Payable in modal
      }
      
    } else {
      if(tempPayAmount == 0){
        setTempPayAmount(Number(payAmount));
      }
        
    }

    setTempMethods(paymentMethods);
  }, [payAmount, paymentMethods, total]);

  useEffect(() => {
    const handleEnterToSaveAndComplete = (e) => {
      if (e.key === "Enter") {
        e.preventDefault();

        // 1️⃣  Run the same logic as your Save button
        const extrasTotal = tempMethods.reduce(
          (sum, m) => sum + (Number(m.payment_amount) || 0),
          0,
        );
        const totalPaid = Number(tempPayAmount || 0) + extrasTotal;

        setPayAmount(Number(tempPayAmount) || 0);
        setPaymentMethods(tempMethods);
        if (setTotalPaidAmount) setTotalPaidAmount(totalPaid);

        const methodNames = [
          selectedGateway?.type_name || "Cash",
          ...tempMethods.map((m) => m.methodName).filter(Boolean),
        ];
        setPaymentName([...new Set(methodNames)].join(" + "));

        // 2️⃣  Close modal first
        setTimeout(() => {
        ...
        setTimeout(() => {
          if (typeof onOrderComplete === "function") {
            onOrderComplete({ status: 1, paidAmount: totalPaid });
          }
        }, 200);
      }
    };

    document.addEventListener("keydown", handleEnterToSaveAndComplete);
    return () =>
      document.removeEventListener("keydown", handleEnterToSaveAndComplete);
  }, [
    tempMethods,
    tempPayAmount,
    selectedGateway,
  ...
  return (
    <div ref={ref} className="space-y-4">
      ...
      <div className="border border-gray-200 p-3 rounded-md bg-white shadow-sm">
        ...
        <Input
          type="number"
          value={isNaN(tempPayAmount) ? "" : tempPayAmount}
          // onChange={(e) => setPayAmount(parseFloat(e.target.value) || 0)} //(commented to fix arif's issue)
          onChange={(e) => setTempPayAmount(parseFloat(e.target.value) || 0)}
        />
      </div>
      ...
      {total ? (
        <div className="border-t pt-3 text-sm text-gray-800 space-y-1">
          {(() => {
            // 🔹 use local temp state (not parent) so preview is live and correct
            const extrasTotal = tempMethods.reduce(
