...
const styles = StyleSheet.create({
  ...
  // --- NEW HEADER STYLES (SaleInvoicePdf4) ---
  ...
  // --- NEW FOOTER STYLES (SaleInvoicePdf4) ---
  footerRoot3: {
    position: "absolute",
    bottom: 0,
    left: 0,
...
function Header({ brandDark, brandLight, logoUrl, invoice, user, qrDataUrl }) {
  const shopAddress = invoice?.data.user_info?.address || "N/A";
  const shopEmail =
    user?.invoice_settings?.email || invoice?.data.user_info?.email || "N/A";
  const invSettings =
    user?.invoice_settings || invoice?.data?.user_info?.invoice_settings || {};
  const shopNumber =
    ...
    <View style={{ marginBottom: 2 }}>
      <View style={styles.headerMainContainer}>
        ...
        <View style={styles.headerCenter3}>
          <Text style={styles.shopTitle3}>{invoice?.data.user_info?.name || "Shop Details"}</Text>
        ...
        <View style={styles.headerRight3}>
          {qrDataUrl && (
            <View style={styles.qrBorder}>
              <Image src={qrDataUrl} style={styles.qrCodeHeader} />
            </View>
          )}
        </View>
      </View>
      ...
      <View style={styles.headerBottomStrip}>
        <View
          style={[styles.headerBottomStripColor1, { backgroundColor: brandDark }]}
        />
        <View
          style={[
            styles.headerBottomStripColor2,
...
function FooterBar({ invoice, user }) {
  const invUser = invoice?.data?.user_info || {};
  const invSet = invUser?.invoice_settings || {};
  const brandDark = invUser?.invoice_settings.second_code || "#111827";

  const website = invUser.web_address || invSet.web_address || "";
  const email = user?.email || invUser?.email || invSet?.email || "";
      ...
      {website && (
        <View style={styles.footerItem3}>
          <Image
            ...
            style={[styles.footerIcon3, { filter: "brightness(0) invert(1)" }]} // Attempting to make icon white
          />
...
// ----- MAIN DOCUMENT -----
export default function SaleInvoicePdf4({
  qrDataUrl,
  invoice,
  user,
  barcodeImage,
  ...
  return (
    <Document>
      <Page size="A4" orientation="portrait" style={styles.page}>
        ...
        {/* Background Image Watermark (CENTERED BODY OVERLAY) */}
        {logoUrl && (
