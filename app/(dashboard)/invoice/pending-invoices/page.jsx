...
import {
  ...
  useSearchPendingInvoicesMutation,
} from "@/app/store/api/pendingInvoicesApi";
import PendingInvoiceFilters from "./PendingInvoiceFilters";
import PendingInvoicePrintSheet from "./pendingInvoicePrint/PendingInvoicePrintSheet";

export default function PendingInvoiceListPage() {
  ...
  const canAccessHoldInvoiceList =
    !isEmployee || canAccess(features, "Sale", "Pending Invoice List");
  ...
  const [completeInvoice, { isLoading: isCompleting }] =
    useCompletePendingInvoiceMutation();
  ...
  const handleCompleteConfirm = async () => {
    ...
    try {
      await completeInvoice({ id: invoiceToComplete.id }).unwrap();
      toast.success(
        `Invoice ${invoiceToComplete.invoice_id} marked complete ✅`,
      );
      refetch();
    } catch (error) {
      console.error(error);
      toast.error("Failed to mark invoice complete.");
    } finally {
  ...
  const fetchAllPendingInvoices = async () => {
    ...
    try {
      ...
      let pages = 1;
      do {
        const res = await api.get(`/pending-list?page=${page}&limit=100`);
  ...
  return (
    <ProtectedRoute featureName="Sale" optionName="Pending Invoice List">
      <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
                      ...
                      return (
                        <React.Fragment key={invoice.id}>
                          <tr className="hover:bg-gray-50">
                            ...
                            <td
                              className={`text-center py-2 font-medium align-middle ${
                                invoice.steadfast_courier?.consignment_id
                                  ? "text-green-600"
                                  : "text-red-500"
                              }`}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Mark Invoice Complete</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to mark{" "}
                <span className="font-semibold">
                  {invoiceToComplete?.invoice_id}
                </span>{" "}
                as completed?
              </AlertDialogDescription>
            </AlertDialogHeader>
