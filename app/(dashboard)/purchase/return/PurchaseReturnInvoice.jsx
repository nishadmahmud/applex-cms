...
import PurchaseReturnInvoicePdf1 from "./PurchaseReturnInvoicePdf1";
import PurchaseReturnInvoicePdf2 from "./PurchaseReturnInvoicePdf2";
import PurchaseReturnPadPrint from "../../invoice/components/PurchaseReturnPadPrint";
import PurchaseReturnPadContainer from "../../invoice/PurchaseReturnPadContainer";
...
const buildPdfBlob = async (design) => {
  const rawLogoUrl =
    purchaseReturn?.user_info?.invoice_settings?.shop_logo || null;

  const logoUrlForPdf = rawLogoUrl
    ? `/api/logo-proxy?url=${encodeURIComponent(rawLogoUrl)}`
    : null;

  const qrDataUrl = await QRCode.toDataURL(purchaseReturn.return_id, {
  ...
  const PdfComponent =
    design === "design1" ? PurchaseReturnInvoicePdf1 : PurchaseReturnInvoicePdf2;
...
const handlePrint = async (design) => {
  ...
  const handlePurchasePrintPad = useReactToPrint({
  contentRef: padRef,
  documentTitle: "Purchase Return Invoice",
});
  ...
  return (
    <Card className="py-10 bg-blue-50 border-0 shadow-none">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="my-4 text-left space-y-1">
              <h3 className="text-base font-bold">Vendor Details:</h3>

              <p className="text-sm">
                <span className="font-bold">Name: </span>
                { purchaseReturn?.vendor != null ? purchaseReturn?.vendor?.name : "-"}
              </p>
              ...
              <p className="text-sm">
                ...
                {purchaseReturn?.vendor != null ? purchaseReturn?.vendor.mobile_number : "-"}
              </p>
          ...
          <div className="flex flex-col items-center flex-1">
            ...
            <p className="text-[10px] font-bold">{purchaseReturn?.return_id}</p>
          </div>
        ...
        {/* BODY: table (use purchaseReturn.return_details or whatever your API returns) */}
        <div className="mb-8 px-5">
          <Table className="border border-gray-400">
            ...
            <TableBody>
              {purchaseReturn?.purchase_details?.map((item) => (
                <TableRow
                  key={item.id}
  ...
  <div className="text-[11px] text-gray-600">
    IMEI: {item.product_imei.imei}
            ...
            <CardContent className="p-0">
              <div className="space-y-2 text-sm">            
                <div className="flex justify-between items-center font-bold text-base">
                  <p className="font-bold text-sm">Returned Amount:</p>
                  <p>
                    ৳{" "}
                    {((purchaseReturn?.return_amount || 0) -
      ...
      <PurchaseReturnPadPrint
        ref={padRef}
        footerText={session?.user?.invoice_settings?.purchase_condition || ""}
      >
      <PurchaseReturnPadContainer
        invoice={purchaseReturn}
        formatProductName={formatProductName}
        return_amount={purchaseReturn?.pay_mode}
        termsData={termsData}
      ></PurchaseReturnPadContainer>
      ...
      <div className="flex flex-wrap gap-4 mt-6 justify-center">
       <div className="relative inline-block">
        ...
        <Button
         onClick={() => handlePurchasePrintPad()}
          variant="outline"
          className="flex items-center bg-transparent"
        >
          <Printer className="w-4 h-4 mr-1" />
