...
export default function AddFooterPopup({ open, onClose }) {
  ...
  const { createFooter, refetch } = useFooter();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createFooter.mutateAsync(payload);
      ...
      toast.error(
        `${error.response?.data?.message}` || "Failed to create footer"
      );
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add New Footer</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <Button type="submit" disabled={isSubmitting} className="flex-1">
              {isSubmitting ? "Creating..." : "Create Footer"}
            </Button>
          </div>
        </form>
      </DialogContent>
