...
const EditMainHeaderPopup = ({ isOpen, onClose, header }) => {
  const dispatch = useDispatch();
  const { data: session, status, update: updateSession } = useSession();
  const { updateHeader } = useMainHeaderQuery();
  ...
  // Initialize form data when header prop changes
  useEffect(() => {
    if (header) {
      setFormData({
        bg_color: header.bg_color || "#1A1A1A",
        status: Boolean(header.status),
        nav_items:
          header.nav_items?.length > 0
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    toast.loading("Updating header...");

    try {
      // Upload icons for nav items that have new files
      ...
      await updateHeader.mutateAsync({ id: header.id, payload });
      toast.dismiss();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Main Header #{header?.id}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div className="space-y-2">
                          ...
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                            {iconPreviews[index] || item.icon ? (
                              <div className="space-y-2">
                                <img
                                  src={iconPreviews[index] || item.icon}
          ...
          <div className="flex justify-end space-x-3 pt-6 border-t">
            ...
            >
              {isLoading ? "Updating..." : "Update Header"}
            </Button>
          </div>
        </form>
      </DialogContent>
...
export default EditMainHeaderPopup;
