...
import { useGetSalesRegisterDetailsReportQuery } from "@/app/store/api/salesRegisterDetailsReportApi";
import SalesRegisterDetailsReportPDF from "./sales-register-details-report-pdf";
...
export default function SalesRegisterDetailsReportPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetSalesRegisterDetailsReportQuery(appliedFilters, {
  ...
  // Flatten rows: one row per sales_details
  const rows = useMemo(() => {
    const out = [];
    items.forEach((inv) => {
      ...
      const details = Array.isArray(inv?.sales_details)
        ? inv.sales_details
        : [];
  ...
  const handleExcelExport = () => {
    ...
    sheetData.push({ Date: "Grand Total", "Sales Amount (BDT)": viewTotal });

    const ws = XLSX.utils.json_to_sheet(sheetData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Sales Register Details");
    XLSX.writeFile(
      wb,
      `sales-register-details-${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
  ...
  const handlePDFExport = async () => {
    ...
    saveAs(
      blob,
      `sales-register-details-${new Date().toISOString().split("T")[0]}.pdf`
    );
  };
  ...
  const handlePrintTable = () => {
    ...
    w.document.write(`
      <html>
        <head>
          <title>Sales Register Details Report</title>
          <style>
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Item-wise sales details within date range
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Sales Register Details</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
