...
export default function PaymentMethodItem({
  ...
  // --------- Autofill first account when method changes ---------
  useEffect(() => {
    if (method && !payment.payment_type_category_id) {
      const firstCat = method.payment_type_category?.[0];
      if (firstCat) {
        onChange(index, { payment_type_category_id: firstCat.id });
      }
    }
  ...
  return (
    <div className="border p-4 rounded-lg bg-gray-50 space-y-3 shadow-sm">
