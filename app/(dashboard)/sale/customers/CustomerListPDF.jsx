  ...
  return (
    <View style={styles.brandHeader}>
      ...
      <View style={styles.brandRight}>
        <Text style={styles.rightText}>Report: Customer List</Text>
...
export default function CustomerListPDF({ data = [], user, logoUrlForPdf }) {
  // Calculate total purchase
  const totalAmount = data.reduce(
    (sum, c) => sum + (Number(c.total_purchase_amount) || 0),
    0
  );
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.title}>Customer List Report</Text>
          ...
          {data.map((c, i) => (
            <View style={styles.tableRow} key={i}>
              ...
              <View style={[styles.tableCol, { width: "17%" }]}>
                <Text style={styles.tableCell}>{c.mobile_number || "-"}</Text>
              </View>
              <View style={[styles.tableCol, { width: "14%" }]}>
                <Text style={styles.tableCell}>
                  {Number(c.total_due_amount || 0).toLocaleString("en-IN")}
                </Text>
              </View>
              ...
              <View style={[styles.tableCol, { width: "25%" }]}>
                <Text style={[styles.tableCell, styles.rightText]}>
                  {Number(c.total_purchase_amount || 0).toLocaleString("en-IN")}
                </Text>
              </View>
            </View>
          ))}
